...
namespace Api.Application.Test.City.RequestGetCompleteById
{
    public class ResponseNotFound
    {
        ...
        public async Task GetResponseNotFound()
        {
            ...
            serviceMock.Setup(x => x.GetCompleteById(It.IsAny<Guid>())).Returns(Task.FromResult((CityCompleteDTO)null));

            _controler = new CitiesController(serviceMock.Object);

            var result = await _controler.GetCompleteById(It.IsAny<Guid>());
