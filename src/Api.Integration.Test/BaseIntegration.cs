...
namespace Api.Integration.Test
{
    public abstract class BaseIntegration : IDisposable
    {
        ...
        public BaseIntegration()
        {
            ...
            myContext =
                server.Host.Services.GetService(typeof (MyContext)) as
                MyContext;
        ...
        {
            var loginDto =
                new LoginDto { Email = "adm@adm.com.br", Password = "123456" };
            var result =
                await PostJsonAsync(loginDto, $"{hostApi}/login", client);

            var jsonResult = await result.Content.ReadAsStringAsync();
            var loginObject =
                JsonConvert.DeserializeObject<LoginResponseDTO>(jsonResult);
